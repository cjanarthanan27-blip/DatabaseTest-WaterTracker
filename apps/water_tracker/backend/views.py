...
from .serializers import (
    ...
    GeneralWaterRateSerializer,
    CategoryStudentCountSerializer,
    ProcurementEntrySerializer,
    BudgetApprovedSerializer,
...
class IsUserManager(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        return request.user and request.user.is_authenticated and (request.user.role == 'Admin' or getattr(request.user, 'can_manage_users', False))
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def reset_password(self, request, pk=None):
        ...
        # If there are any pending reset requests for this user, resolve them
        pending_requests = PasswordResetRequest.objects.filter(user=user, status='Pending')
        ...
        return Response({'message': f'Password for {user.username} successfully reset.'}, status=status.HTTP_200_OK)
...
class YieldEntryViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # [NEW] Check for manual previous reading override
        is_manual_previous = serializer.initial_data.get('is_manual_previous', False)
        
        if is_manual_previous:
            # If override is on, use the provided previous_reading from initial_data
            try:
                previous_reading = Decimal(str(serializer.initial_data.get('previous_reading', 0)))
            except (ValueError, TypeError, InvalidOperation):
                previous_reading = Decimal('0')
        else:
            # Otherwise, get it from the latest entry Strictly BEFORE this one's date or ID
            last_entry = YieldEntry.objects.filter(
                location=location,
                date__lt=date
            ).order_by('-date', '-created_at').first()
            
            # If no older date, check if there's an entry on same date (in case of sub-day entries)
            if not last_entry:
                last_entry = YieldEntry.objects.filter(
                    location=location,
                    date=date
        ...
        if location.is_manual_yield:
            ...
            yield_liters_raw = serializer.initial_data.get('yield_liters')
            try:
                if yield_liters_raw is not None and yield_liters_raw != '':
                    yield_liters = Decimal(str(yield_liters_raw))
                else:
                    yield_liters = serializer.validated_data.get('yield_liters', Decimal('0'))
            except (ValueError, TypeError, InvalidOperation):
                yield_liters = Decimal('0')
        else:
        ...
        try:
            with transaction.atomic():
                for entry in entries_data:
                    ...
                    # [NEW] Check for manual previous reading override
                    is_manual_previous = entry.get('is_manual_previous', False)
                    
                    if is_manual_previous:
                        try:
                            previous_reading = Decimal(str(entry.get('previous_reading', 0)))
                        except (ValueError, TypeError, InvalidOperation):
                            previous_reading = Decimal('0')
                    else:
                        prev_entry = (
                    ...
                    yield_liters_to_save = Decimal('0')
                    if location.is_manual_yield:
                        yield_liters_to_save = Decimal(str(entry.get("yield_liters") or 0))
                    else:
                        yield_diff = (
                        ...
                        yield_liters_to_save = yield_diff * Decimal('1000')
    ...
    def perform_update(self, serializer):
        ...
        if is_manual_previous:
            ...
            # If no older date, check if there's an entry on same date excluding self
            if not last_entry:
                last_entry = YieldEntry.objects.filter(
                    location=location,
                    date=date
                ).order_by('-created_at').first() # Simplified: just get latest for that day if no LT entry
                
                # If we found an entry on the same date, ensure it's not the one we're updating
                if last_entry and last_entry.id == serializer.instance.id:
                     last_entry = YieldEntry.objects.filter(
                        location=location,
                        date=date
                    ).exclude(id=serializer.instance.id).order_by('-created_at').first()
        ...
        if location.is_manual_yield:
            ...
            try:
                ...
                yield_liters = serializer.instance.yield_liters
        else:
            yield_difference = (current_reading - previous_reading 
                              if current_reading > previous_reading else Decimal('0'))
            yield_liters = yield_difference * Decimal('1000')
    ...
    def get(self, request):
        ...
        if date_str:
            try:
                ...
                filters["date__lt"] = look_date  # previous reading is strictly before
            except ValueError:
                pass
            ...
            results.append(
                {
                    ...
                    "previous_reading": prev_entry.current_reading if prev_entry else Decimal('0'),
                    "current_reading": existing_entry.current_reading if existing_entry else "",
                    "comments": existing_entry.comments if existing_entry else "",
                }
            )
                ...
                for entry in entries_data:
                    ...
                    entry_obj, created = ConsumptionEntry.objects.update_or_create(
                        ...
                        defaults={
                            "current_reading": current_reading,
                            "previous_reading": previous_reading,
                            "consumption_liters": consumption_diff * Decimal('1000'),
                            "comments": comments,
                            "created_by": (
                                request.user if request.user.is_authenticated else None
                            ),
...
class GeneralWaterRateViewSet(viewsets.ModelViewSet):
    queryset = GeneralWaterRate.objects.all().order_by('-effective_date', '-date')
    serializer_class = GeneralWaterRateSerializer

    def perform_create(self, serializer):
        serializer.save(created_by=self.request.user)


class CategoryStudentCountViewSet(viewsets.ModelViewSet):
    queryset = CategoryStudentCount.objects.all().order_by('-effective_date', '-entered_at')
    serializer_class = CategoryStudentCountSerializer
    pagination_class = None
    ...
    try:
        ...
        normal_entries_breakdown = (
            WaterEntry.objects.filter(
                entry_date__gte=start_of_month,
                water_type="Normal Water (Salt)",
                loading_location_id=19,  # Muthu Nagar Well
                vehicle_id__in=[1, 2],   # Rathinam Eicher (12000) & Tractor (6000)
            )
            .exclude(source__source_type="Pipeline")
        ...
        for entry in normal_entries_breakdown:
            ...
            # Update totals
            liters = float(entry.total_quantity_liters)
            amount = float(entry.total_cost)

            nw_location_data[loc_name]["total_liters"] += liters
            nw_location_data[loc_name]["total_amount"] += amount
            nw_grand_total["total_liters"] += liters
            nw_grand_total["total_amount"] += amount

            # Determine Load Size (12KL or 6KL) based on vehicle
            load_count = entry.load_count if entry.load_count else 1
            if entry.vehicle_id == 2:  # Rathinam Eicher (12000)
                nw_location_data[loc_name]["count_12kl"] += load_count
                nw_grand_total["count_12kl"] += load_count
            elif entry.vehicle_id == 1:  # Rathinam Tractor (6000)
                nw_location_data[loc_name]["count_6kl"] += load_count
                nw_grand_total["count_6kl"] += load_count
        ...
        for entry in bannari_entries_breakdown:
            ...
            bw_location_data[loc_name]["total_liters"] += liters
            bw_location_data[loc_name]["total_amount"] += amount
            bw_grand_total["total_liters"] += liters
            bw_grand_total["total_amount"] += amount
            ...
            if avg_load_size >= 10000:
                ...
                bw_grand_total["count_12kl"] += load_count
            elif avg_load_size >= 4000:
                bw_location_data[loc_name]["count_6kl"] += load_count
                bw_grand_total["count_6kl"] += load_count
        ...
        return Response(
            {
                ...
                "normal_water_breakdown_total": nw_grand_total,
                "bannari_water_breakdown": bannari_water_breakdown,
                "bannari_water_breakdown_total": bw_grand_total,
...
class ProcurementEntryViewSet(viewsets.ModelViewSet):
    queryset = ProcurementEntry.objects.all().order_by('-created_at')
    serializer_class = ProcurementEntrySerializer

    def get_queryset(self):
        qs = super().get_queryset()
        status_filter = self.request.query_params.get('payment_status')
        month = self.request.query_params.get('for_month')
        if status_filter:
            qs = qs.filter(payment_status=status_filter)
        if month:
            qs = qs.filter(for_month__startswith=month)
        return qs

    @action(detail=False, methods=['get'])
    def vendors(self, request):
        """Return distinct non-empty vendor names for autocomplete."""
        names = (
            ProcurementEntry.objects
            .exclude(vendor='')
    ...
    def blocks(self, request):
        """Return distinct non-empty block_details values for autocomplete."""
        values = (
            ProcurementEntry.objects
            .exclude(block_details='')
            .values_list('block_details', flat=True)
            .distinct()
            .order_by('block_details')
        )
        return Response(list(values))

    @action(detail=False, methods=['get'])
    def categories(self, request):
        """Return distinct non-empty category values for autocomplete."""
        values = (
            ProcurementEntry.objects
            .exclude(category='')
...
class BudgetApprovedViewSet(viewsets.ModelViewSet):
    queryset = BudgetApproved.objects.all().order_by('-for_month')
    serializer_class = BudgetApprovedSerializer
...
class RequestPasswordResetView(APIView):
    ...
    def post(self, request):
        ...
        if not user:
            ...
            return Response({"message": "If the account exists, a reset request has been logged."}, status=status.HTTP_200_OK)
        ...
        if existing_request:
            return Response({"message": "A password reset request is already pending for this account."}, status=status.HTTP_200_OK)

        # Create the request
        PasswordResetRequest.objects.create(user=user)

        return Response({"message": "Password reset requested. An administrator will contact you soon."}, status=status.HTTP_201_CREATED)


class AdminPasswordResetViewSet(viewsets.ModelViewSet):
    queryset = PasswordResetRequest.objects.all().order_by('-requested_at')
    serializer_class = PasswordResetRequestSerializer
    # Ensure only Admin can access this ViewSet
    # Assuming role is handled via standard permissions, but explicitly checking here just in case:
    def get_permissions(self):
        # Allow default rules, but we'll check role in the action
    ...
    def resolve(self, request, pk=None):
        if request.user.role != 'Admin':
             return Response({"error": "Not authorized to resolve password resets"}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response({"message": f"Password for {user.username} has been updated successfully."}, status=status.HTTP_200_OK)
...
class DatabaseResetView(APIView):
    ...
    def post(self, request):
        if not getattr(request.user, 'role', None) == 'Admin':
            return Response({"error": "Only admins can reset the database"}, status=status.HTTP_403_FORBIDDEN)

        try:
            # Using 'flush' is safer than deleting the file on Windows
            # It clears all data from all tables but keeps the structure
            call_command('flush', interactive=False, reset_sequences=True)

            # 4. Create default admin (must be done immediately as flush deleted all users)
            User = get_user_model()
            ...
            return Response({
                "message": "Database reset successful (data flushed). Default admin created.",
