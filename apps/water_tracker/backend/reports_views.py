...
from .models import (
    ...
    GeneralWaterRate, CategoryStudentCount, ProcurementEntry, BudgetApproved
)
import calendar
        ...
        try:
            ...
            for month_date in months:
                ...
                for cat in categories:
                    ...
                    # 1st count (student_count) → normal water
                    # 2nd count (second_count)  → drinking water
                    # Falls back to student_count for drinking if second_count not set
                    sc_entry = CategoryStudentCount.objects.filter(
                        category=cat,
                        effective_date__lte=month_date.replace(day=days_in_month)
                    ).order_by('-effective_date', '-entered_at').first()

                    if sc_entry:
                        normal_student_count = sc_entry.student_count or 0
                        drinking_student_count = (
                            sc_entry.second_count
                            if sc_entry.second_count
                            else normal_student_count
                        )
                    else:
                        normal_student_count = 0
                        drinking_student_count = 0

                    # Respect has_student_count flag on category
                    if not cat.has_student_count:
                        normal_student_count = 0
                        drinking_student_count = 0
                ...
                # 4b. Budget vs Cumulative Procurement Spend
                def get_fy_start(d):
                    if d.month >= 4:
                ...
                fy_start = get_fy_start(month_date)
                
                budget_entry = BudgetApproved.objects.filter(for_month=month_date).first()
                approved_budget = budget_entry.amount if budget_entry else Decimal('0')
                
                # Cumulative spend from April of this FY to this month
                cum_spend_agg = ProcurementEntry.objects.filter(
                    for_month__gte=fy_start,
                    for_month__lte=month_date
                ).aggregate(total=Sum('opex_value'))
                cumulative_spend = cum_spend_agg['total'] or Decimal('0')
                
                month_item['budget_spend'] = {
...
class ProcurementReportView(APIView):
    """
    Procurement Tracker Report - Grouped by for_month
    Query params: ?start_month=2024-01&end_month=2024-12
    """
    def get(self, request):
        try:
            ...
            entries = ProcurementEntry.objects.all().order_by('for_month', 'created_at')
            
            if start_month_str:
                start_date = datetime.strptime(start_month_str + '-01', '%Y-%m-%d').date()
                entries = entries.filter(for_month__gte=start_date)
            if end_month_str:
                # To include the whole end_month, we set to the first day of the NEXT month
                # but for for_month we use the first day of the month consistently.
                # So we just filter for_month__lte first day of end_month.
                end_date = datetime.strptime(end_month_str + '-01', '%Y-%m-%d').date()
                entries = entries.filter(for_month__lte=end_date)

            # Process in memory to maintain groups easily for the frontend
            grouped_data = []
            current_month = None
            current_group = None

            for entry in entries:
                month_val = entry.for_month
                month_key = month_val.strftime('%Y-%m') if month_val else 'None'
                ...
                if month_key != current_month:
                    ...
                    month_label = month_val.strftime("%b'%y") if month_val else 'NA'
                    current_group = {
                ...
                opex_val = entry.opex_value or Decimal('0')
                current_group['month_total'] += opex_val
            ...
            return Response({
                'monthly_data': grouped_data,
                'grand_total': sum(item['month_total'] for item in grouped_data)
            }, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


class MuthuNagarBuildingWiseReportView(APIView):
    """
    Building Wise Water Purchase from Muthu Nagar well using Rathinam vehicle
    ...
    def get(self, request):
        try:
            ...
            entries = WaterEntry.objects.filter(
                ...
                water_type="Normal Water (Salt)"
            ).exclude(source__source_type="Pipeline").select_related("unloading_location")
            ...
            for entry in entries:
                ...
                location_data[loc_name]["total_liters"] += liters
                location_data[loc_name]["total_amount"] += amount
                grand_total["total_liters"] += liters
                ...
                if entry.vehicle_id == 2:  # Rathinam Eicher (12000)
                    location_data[loc_name]["count_12kl"] += load_count
                    grand_total["count_12kl"] += load_count
                elif entry.vehicle_id == 1:  # Rathinam Tractor (6000)
                    location_data[loc_name]["count_6kl"] += load_count
                    grand_total["count_6kl"] += load_count

            # Convert to sorted list
            report_data = list(location_data.values())
...
class BannariBuildingWiseReportView(APIView):
    """
    Building Wise Water Purchase from Bannari Water
    ...
    def get(self, request):
        try:
            ...
            for entry in entries:
                ...
                # Determine Load Size (>=10000 for 12KL, >=4000 for 6KL)
                load_count = entry.load_count if entry.load_count else 1
                avg_load_size = liters / load_count
