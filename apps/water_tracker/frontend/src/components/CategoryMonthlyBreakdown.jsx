...
import { exportFullReportToPDF, formatDateRange } from '../utils/pdfExport';
...
const CategoryMonthlyBreakdown = () => {
    ...
    const handleExportPDF = async () => {
        ...
        const elementIds = [
            ...
            data && data.length > 0 ? 'category-summary-section' : null,
            data && data.length > 0 ? 'category-amount-summary-section' : null,
            data && data.length > 0 && data.some(m => m.budget_spend) ? 'category-budget-summary-section' : null,
            data && data.length > 0 && data.some(m => m.institute_summary) ? 'category-institute-summary-section' : null,
            data && data.length > 0 && data.some(m => m.schools_summary) ? 'category-schools-summary-section' : null,
            data && data.length > 0 && data.some(m => m.hostels_summary) ? 'category-hostels-summary-section' : null,
            data && data.length > 0 && data.some(m => m.kitchen_summary) ? 'category-kitchen-summary-section' : null,
        ].filter(Boolean);

        const result = await exportFullReportToPDF(
    ...
    return (
        <div className="space-y-6">
            ...
            {data && data.length > 0 && data.some(m => m.budget_spend) && (
                <div id="category-budget-summary-section" className="space-y-8 mt-12 pt-12 border-t-4 border-gray-100 dark:border-slate-700">
                    <h2 className="text-3xl font-black text-center text-gray-900 dark:text-slate-100 uppercase tracking-tight break-words max-w-4xl mx-auto">WATER</h2>
                    <h3 className="text-xl font-bold text-center text-gray-600 dark:text-slate-400 -mt-4 break-words max-w-4xl mx-auto">Budget</h3>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700">
                        <ResponsiveContainer width="100%" height={400}>
                            <BarChart
                                data={[
                                    { name: 'Approved Budget', ...data.reduce((acc, m) => ({ ...acc, [m.month_name]: m.budget_spend?.approved_budget || 0 }), {}) },
                                    { name: 'Cumulative Budget spent (Actual expenses) till now', ...data.reduce((acc, m) => ({ ...acc, [m.month_name]: m.budget_spend?.cumulative_spend || 0 }), {}) },
                                ]}
                                margin={{ top: 20, right: 30, left: 20, bottom: 60 }}
                            >
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                                ...
                                {data.map((month, index) => (
                                    <Bar
                                        key={month.month_key}
                                        dataKey={month.month_name}
                                        fill={['#3b82f6', '#ec4899', '#10b981', '#8b5cf6', '#d97706', '#06b6d4'][index % 6]}
                                        radius={[4, 4, 0, 0]}
                                    />
                                ))}
                            </BarChart>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                        <table className="min-w-full border-collapse text-sm">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-slate-900/50">
                                    <th className="px-4 py-3 border dark:border-slate-700 text-left font-bold text-gray-500 uppercase">Period</th>
                                    <th className="px-4 py-3 border dark:border-slate-700 text-center font-bold text-gray-500 uppercase">Approved Budget</th>
                                    <th className="px-4 py-3 border dark:border-slate-700 text-center font-bold text-gray-500 uppercase">Cumulative Budget spent (Actual expenses) till now</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                                {data.map((month, index) => {
                                    const dotColor = ['bg-blue-500', 'bg-pink-500', 'bg-emerald-500', 'bg-violet-500', 'bg-amber-600', 'bg-cyan-500'][index % 6];
                                    return (
                                        <tr key={month.month_key} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                            <td className="px-4 py-3 border dark:border-slate-700 font-medium text-gray-900 dark:text-slate-100 whitespace-nowrap">
                                                <span className={`inline-block w-3 h-3 rounded-full mr-2 ${dotColor}`}></span>
                                            ...
                                            <td className="px-4 py-3 border dark:border-slate-700 text-right">{month.budget_spend?.approved_budget.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}</td>
                                            <td className="px-4 py-3 border dark:border-slate-700 text-right">{month.budget_spend?.cumulative_spend.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) || 0}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
            ...
            {data && data.length > 0 && data.some(m => m.institute_summary) && (
                <div id="category-institute-summary-section" className="space-y-8 mt-12 pt-12 border-t-4 border-gray-100 dark:border-slate-700">
            ...
            {data && data.length > 0 && data.some(m => m.schools_summary) && (
                <div id="category-schools-summary-section" className="space-y-8 mt-12 pt-12 border-t-4 border-gray-100 dark:border-slate-700">
            ...
            {data && data.length > 0 && data.some(m => m.hostels_summary) && (
                <div id="category-hostels-summary-section" className="space-y-8 mt-12 pt-12 border-t-4 border-gray-100 dark:border-slate-700">
            ...
            {data && data.length > 0 && data.some(m => m.kitchen_summary) && (
                <div id="category-kitchen-summary-section" className="space-y-8 mt-12 pt-12 border-t-4 border-gray-100 dark:border-slate-700">
