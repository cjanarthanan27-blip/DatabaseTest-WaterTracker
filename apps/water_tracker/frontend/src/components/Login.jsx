    ...
    const handleSubmit = async (e) => {
        ...
        if (isForgotPassword) {
            try {
                ...
                setTimeout(() => setIsForgotPassword(false), 3000); // return to login after 3s
            } catch (err) {
            ...
            <div className="max-w-md w-full space-y-8 bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700">
                <div>
                    <h2 className="text-center text-3xl font-extrabold text-gray-900 dark:text-slate-100">
                        {isForgotPassword ? 'Reset Password' : 'Sign in to Water Tracker'}
                    </h2>
                    <p className="mt-2 text-center text-sm text-gray-600 dark:text-slate-400">
                        {isForgotPassword
                            ? 'Submit your username or email to request a new password from an administrator.'
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <div className="text-sm">
                            <button
                                ...
                                className="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300 bg-transparent border-none p-0 cursor-pointer"
                            >
                    ...
                    {successMessage && (
                        <div className="text-green-600 dark:text-green-400 text-sm text-center font-medium bg-green-50 dark:bg-green-900/20 py-2 rounded">
                            {successMessage}
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        >
                            {isForgotPassword ? 'Request Reset' : 'Sign in'}
                        </button>
                    </div>
                </form>
            </div>
