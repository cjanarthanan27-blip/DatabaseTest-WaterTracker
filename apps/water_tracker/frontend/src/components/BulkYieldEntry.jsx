...
const BulkYieldEntry = () => {
    ...
    const handleReadingChange = (index, value) => {
        ...
        // Auto-calculate for manual yield locations when reading changes
        if (newRows[index].is_manual_yield && value && !isNaN(value)) {
            const diff = parseInt(value) - newRows[index].previous_reading;
            if (diff >= 0) {
                newRows[index].yield_liters = (diff * 1000).toString();
            }
        }
    ...
    const handleOverrideChange = (index, checked) => {
        const newRows = [...rows];
        newRows[index].is_manual_previous = checked;
        setRows(newRows);
    };

    const handlePreviousReadingChange = (index, value) => {
        const newRows = [...rows];
        newRows[index].previous_reading = value;

        // Recalculate yield if current reading exists
        if (!newRows[index].is_manual_yield && newRows[index].current_reading) {
            // Auto recalc happens in render or can be manual here
        }
        setRows(newRows);
    };
        ...
        try {
            await api.post('yield-entries/bulk_create/', {
                ...
                entries: validEntries.map(r => ({
                    ...
                    is_manual_previous: r.is_manual_previous,
                    yield_liters: r.is_manual_yield ? parseFloat(r.yield_liters || 0) : null,
                    comments: r.comments
                }))
            });
    ...
    const calculateYield = (row) => {
        ...
        const diff = parseFloat(row.current_reading) - parseFloat(row.previous_reading);
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-blue-900 dark:text-white">Bulk Yield Entry</h1>
                <button
                    onClick={() => navigate('/yield-entries')}
                    className="text-blue-600 dark:text-gray-400 hover:text-blue-800 dark:hover:text-white font-medium transition-colors"
                >
                    Back to List
                </button>
            </div>

            <div className="bg-white dark:bg-slate-800 shadow-xl rounded-2xl p-6 mb-6 border border-blue-50 dark:border-slate-700">
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        <thead className="bg-gray-50 dark:bg-slate-900/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Override</th>
                                ...
                                <tr key={row.location_id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center">
                                        {parseFloat(row.previous_reading) === 0 && (
                                            <input
                                                type="checkbox"
                                                checked={row.is_manual_previous}
                                                onChange={(e) => handleOverrideChange(index, e.target.checked)}
                                                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                                title="Override previous reading"
                                            />
                                        )}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-slate-400 bg-gray-50 dark:bg-slate-900/20">
                                        {row.is_manual_previous ? (
                                            <input
                                                type="number"
                                                step="0.0001"
                                                value={row.previous_reading}
                                                onChange={(e) => handlePreviousReadingChange(index, e.target.value)}
                                                className="w-24 px-2 py-1 border border-blue-300 dark:border-blue-700 rounded focus:ring-2 focus:ring-blue-500 dark:bg-slate-900 dark:text-white text-sm"
                                            />
                                        ) : (
