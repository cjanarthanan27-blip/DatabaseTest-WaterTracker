...
const AddEntry = () => {
    ...
    const [entryFetching, setEntryFetching] = useState(false);
    ...
    useEffect(() => {
        if (!isEditMode && entryType === 'Internal' && sources.length > 0) {
    ...
    }, [entryType, sources, formData.source, isEditMode]); // Dependent on entryType switching
            ...
            if (mainTab === 'Yield' && yieldFormData.location && !isEditMode) {
                try {
                    ...
                    setYieldFormData(prev => ({
                        ...prev,
                        previous_reading: response.data.previous_reading || 0,
                        is_manual_previous: false // Reset override on location change
                    }));
                } catch (err) {
            ...
            const fetchEntry = async () => {
                ...
                try {
                    if (isYieldEdit) {
                        ...
                        } else if (entry.vehicle) {
                            type = 'Internal'; // If no source but has vehicle, it's internal
    ...
    useEffect(() => {
        ...
        if (isEditMode && (dataLoading || entryFetching)) return;
    ...
    }, [formData.source, formData.vehicle, formData.loading_location, formData.total_quantity_liters, formData.entry_date, sources, isEditMode, dataLoading, entryFetching, formData.load_count, formData.total_cost, formData.water_type, entryType, isManualOverride]);
    ...
    }, [formData.vehicle, formData.load_count, vehicles, formData.total_quantity_liters, isManualOverride, formData.manual_capacity_liters, isEditMode, dataLoading, entryFetching]);
    ...
    const handleYieldSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            // Sanitize payload: convert numeric strings to actual numbers or null
            const payload = {
                ...yieldFormData,
                current_reading: yieldFormData.current_reading ? parseFloat(yieldFormData.current_reading) : 0,
                previous_reading: yieldFormData.previous_reading ? parseFloat(yieldFormData.previous_reading) : 0,
                yield_liters: (yieldFormData.yield_liters !== '' && yieldFormData.yield_liters !== null)
                    ? parseFloat(yieldFormData.yield_liters)
                    : null
            };

            if (isEditMode && isYieldEdit) {
                await api.put(`yield-entries/${id}/`, payload);
                setToast({ message: 'Yield entry updated successfully!', type: 'success' });
            } else {
                await api.post('yield-entries/', payload);
    ...
    const handleConsumptionSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...consumptionFormData,
                current_reading: consumptionFormData.current_reading ? parseFloat(consumptionFormData.current_reading) : 0,
                previous_reading: consumptionFormData.previous_reading ? parseFloat(consumptionFormData.previous_reading) : 0
            };
            ...
            if (isEditMode && isConsumptionEdit) {
                ...
                await api.post('consumption-entries/', payload);
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8">
            <div className="mb-6">
                ...
                <p className="text-blue-600/70 dark:text-slate-400 mt-1">
            ...
            {/* Main Tabs */}
            {!isEditMode && (
                <div className="flex bg-blue-50/50 dark:bg-slate-900/50 p-1.5 rounded-xl mb-6 w-fit border border-blue-100 dark:border-slate-700 shadow-sm">
                    <button
                        onClick={() => setMainTab('Purchase')}
                        className={`px-6 py-2.5 rounded-lg transition-all font-semibold text-sm ${mainTab === 'Purchase'
                            ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-white shadow-md ring-1 ring-blue-100 dark:ring-slate-600'
                            : 'text-blue-600/60 hover:text-blue-700 dark:text-slate-400'
                            }`}
                    >
                        Water Purchase
                    </button>
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-lg transition-all font-semibold text-sm ${mainTab === 'Yield'
                            ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-white shadow-md ring-1 ring-blue-100 dark:ring-slate-600'
                            : 'text-blue-600/60 hover:text-blue-700 dark:text-slate-400'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-lg transition-all font-semibold text-sm ${mainTab === 'Consumption'
                            ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-white shadow-md ring-1 ring-blue-100 dark:ring-slate-600'
                            : 'text-blue-600/60 hover:text-blue-700 dark:text-slate-400'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-lg transition-all font-semibold text-sm ${mainTab === 'GeneralRates'
                            ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-white shadow-md ring-1 ring-blue-100 dark:ring-slate-600'
                            : 'text-blue-600/60 hover:text-blue-700 dark:text-slate-400'
                            }`}
                    >
            ...
            {mainTab === 'Purchase' && (
                <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 shadow-xl rounded-2xl p-8 space-y-8 border border-blue-50 dark:border-slate-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {Number(yieldFormData.previous_reading) === 0 && (
                                    <label className="flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={yieldFormData.is_manual_previous}
                                            onChange={(e) => setYieldFormData({ ...yieldFormData, is_manual_previous: e.target.checked })}
                                            className="w-4 h-4 text-blue-600 border-gray-300 dark:border-slate-600 rounded focus:ring-blue-500 bg-white dark:bg-slate-800"
                                        />
                                        <span className="ml-2 text-[10px] text-gray-500 dark:text-slate-400">Override</span>
                                    </label>
                                )}
                            </div>
                            <input
                                ...
                                onChange={(e) => setYieldFormData({ ...yieldFormData, previous_reading: e.target.value })}
                                readOnly={!yieldFormData.is_manual_previous}
                                className={`w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 dark:text-white ${!yieldFormData.is_manual_previous ? 'bg-gray-50 dark:bg-slate-900/50 text-gray-500' : ''}`}
                            />
                            {!yieldFormData.is_manual_previous && <p className="mt-1 text-[10px] text-gray-400">Automatically fetched from last entry</p>}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {Number(consumptionFormData.previous_reading) === 0 && (
                                    <label className="flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={consumptionFormData.is_manual_previous}
                                            onChange={(e) => setConsumptionFormData({ ...consumptionFormData, is_manual_previous: e.target.checked })}
                                            className="w-4 h-4 text-blue-600 border-gray-300 dark:border-slate-600 rounded focus:ring-blue-500 bg-white dark:bg-slate-800"
                                        />
                                        <span className="ml-2 text-[10px] text-gray-500 dark:text-slate-400">Override</span>
                                    </label>
                            ...
                            <input
                                ...
                                onChange={(e) => setConsumptionFormData({ ...consumptionFormData, previous_reading: e.target.value })}
                                readOnly={!consumptionFormData.is_manual_previous}
                                className={`w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-white ${!consumptionFormData.is_manual_previous ? 'bg-gray-50 dark:bg-slate-900/50 text-gray-500 dark:text-slate-400' : ''}`}
                            />
                            {!consumptionFormData.is_manual_previous && <p className="mt-1 text-xs text-blue-600 dark:text-blue-400">Read-only (auto-calculated)</p>}
                        </div>
